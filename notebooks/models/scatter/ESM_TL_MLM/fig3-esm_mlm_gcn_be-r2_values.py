#!/usr/bin/env python
"""
PyTorch Lightning ESM-MLM-GCN BE model runner for R^2 values.
"""
import argparse
import os
import torch
...
class LightningEsmGcnBe(L.LightningModule):
    def __init__(self, 
                 ...
                 max_len: int, mask_prob: float, gcn_model: GraphSAGE_BE, esm_version="facebook/esm2_t6_8M_UR50D"):
        super().__init__()
    ...
    def step(self, batch):
        ...
        _, binding_preds, expression_preds, y = self(input_ids=masked_original_ids, attention_mask=attention_mask, binding_targets=binding_targets, expression_targets=expression_targets)
    ...
    def on_fit_end(self):
        ...
        save_as = f"esm_mlm_gcn_be/esm_mlm_gcn_be.{self.script_letter}-predicted_vs_measured.values.csv"

#!/usr/bin/env python
"""
PyTorch Lightning ESM-MLM-GCN model runner for R^2 values.
"""
import argparse
import os
import torch
...
class LightningEsmGcn(L.LightningModule):
    def __init__(self, 
                 ...
                 max_len: int, mask_prob: float, gcn_model: GraphSAGE, esm_version="facebook/esm2_t6_8M_UR50D"):
        super().__init__()
    ...
    def step(self, batch):
        ...
        _, preds, y = self(input_ids=masked_original_ids, attention_mask=attention_mask, targets=targets)
    ...
    def on_fit_end(self):
        ...
        save_as = f"esm_mlm_gcn/esm_mlm_gcn.{self.script_letter}.{self.binding_or_expression}-predicted_vs_measured.values.csv"

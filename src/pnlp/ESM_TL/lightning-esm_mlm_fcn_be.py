#!/usr/bin/env python
"""
PyTorch Lightning ESM-MLM-FCN BE (with masked language head) model runner.
"""
import argparse
import os
import time
...
class LightningEsmFcnBe(L.LightningModule):
    def __init__(self, 
                 ...
                 lr: float, max_len: int, mask_prob: float, fcn_model: FCN_BE, esm_version="facebook/esm2_t6_8M_UR50D", freeze_esm_weights=False, from_esm_mlm=None):        
        super().__init__()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run ESM-MLM-FCN BE model with Lightning")
    ...
    logger = CSVLogger(save_dir="logs", name=f"esm_mlm_fcn_be", version=f"version_{slurm_job_id}" if slurm_job_id is not None else None)

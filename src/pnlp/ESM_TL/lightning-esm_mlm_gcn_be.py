#!/usr/bin/env python
"""
PyTorch Lightning ESM-MLM-GCN BE (with masked language head) model runner.
""" 
import argparse
import os
import time
...
class LightningEsmGcnBe(L.LightningModule):
    def __init__(self, 
                 ...
                 lr: float, max_len: int, mask_prob: float, gcn_model: GraphSAGE_BE, esm_version="facebook/esm2_t6_8M_UR50D", freeze_esm_weights=False, from_esm_mlm=None):        
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, binding_targets, expression_targets):
        ...
        binding_gcn_output, expression_gcn_output = self.gcn(batch_graph.x, batch_graph.edge_index, batch_graph.batch)

        return esm_output, binding_gcn_output, expression_gcn_output, batch_graph.y
    ...
    def step(self, batch):
        ...
        mlm_output, binding_regression_preds, expression_regression_preds, y = self(input_ids=masked_original_ids, attention_mask=attention_mask, binding_targets=binding_targets, expression_targets=expression_targets)
        ...
        binding_regression_loss = self.regression_loss_fn(binding_regression_preds, y[:, 0])  # Sum of squared errors (sse)
        expression_regression_loss = self.regression_loss_fn(expression_regression_preds, y[:, 1])  # Sum of squared errors (sse)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run ESM-MLM-GCN BE model with Lightning")
    ...
    logger = CSVLogger(save_dir="logs", name=f"esm_mlm_gcn_be", version=f"version_{slurm_job_id}" if slurm_job_id is not None else None)

#!/usr/bin/env python
"""
PyTorch Lightning ESM-MLM-BLSTM (with masked language head) model runner.
"""
import argparse
import os
import time
...
class LightningEsmBlstm(L.LightningModule):
    def __init__(self, 
                 ...
                 lr: float, max_len: int, mask_prob: float, blstm_model: BLSTM, esm_version="facebook/esm2_t6_8M_UR50D", freeze_esm_weights=False, from_esm_mlm=None):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask):
        ...
        return esm_output, self.blstm(esm_aa_embedding)  
    ...
    def step(self, batch):
        ...
        mlm_output, regression_preds = self(input_ids=masked_original_ids, attention_mask=attention_mask)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run ESM-MLM-BLSTM model with Lightning")
    ...
    logger = CSVLogger(save_dir="logs", name=f"esm_mlm_blstm", version=f"version_{slurm_job_id}" if slurm_job_id is not None else None)

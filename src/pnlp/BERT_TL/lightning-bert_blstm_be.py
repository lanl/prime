#!/usr/bin/env python
"""
PyTorch Lightning BERT-BLSTM BE model runner.
"""
import argparse
import os
import time
...
class LightningEsmBlstmBe(L.LightningModule):
    ...
    def step(self, batch):
        ...
        mlm_preds, binding_regression_preds, expression_regression_preds = self(masked_tokenized_seqs)
...
if __name__ == "__main__":
    ...
    logger = CSVLogger(save_dir="logs", name=f"bert_blstm_be", version=f"version_{slurm_job_id}" if slurm_job_id is not None else None)

#!/usr/bin/env python
"""
PyTorch Lightning BERT-GCN model runner.
"""
import argparse
import os
import time
...
class LightningBertGcn(L.LightningModule):
    def __init__(self, 
                 binding_or_expression:str, from_checkpoint:str, # Only set for hparams save
                 lr: float, max_len: int, mask_prob:float, gcn_model: GraphSAGE, 
    ...
    def forward(self, masked_tokenized_seqs, targets):
        bert_output = self.bert(masked_tokenized_seqs)

        # Graph Construction
        graphs = []
        for embedding, target in zip(bert_output, targets):
        ...
        return self.mlm_model(bert_output), output, batch_graph.y
    ...
    def step(self, batch):
        ...
        mlm_preds, regression_preds, y = self(masked_tokenized_seqs, targets)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run BERT-GCN model with Lightning")
    ...
    logger = CSVLogger(save_dir="logs", name=f"bert_gcn", version=f"version_{slurm_job_id}" if slurm_job_id is not None else None)

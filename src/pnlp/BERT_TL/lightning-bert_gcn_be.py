#!/usr/bin/env python
"""
PyTorch Lightning BERT-GCN BE model runner.
"""
import argparse
import os
import time
...
class LightningBertGcnBe(L.LightningModule):
    def __init__(self, 
                 ...
                 lr: float, max_len: int, mask_prob:float, gcn_model: GraphSAGE_BE, 
    ...
    def step(self, batch):
        ...
        mlm_preds, binding_regression_preds, expression_regression_preds, y = self(masked_tokenized_seqs, binding_targets, expression_targets)
...
if __name__ == "__main__":
    ...
    logger = CSVLogger(save_dir="logs", name=f"bert_gcn_be", version=f"version_{slurm_job_id}" if slurm_job_id is not None else None)
